...
$.extend($.ui.dialog.overlay, {
	...
	//	alert(Object.toJSON(this.instances));
		return $el;
	},
...
MC.prototype={
  ...
  initialize:function(){
    ...
    this. timeoutId=null;
    this.mcount=null;
    jQuery(document.body).append("<div id=\"dialog\" title=\""+"\u901a\u77e5"+"\" style=\"display:none;background-color:white;overflow:hidden;\"><div id='dialog-dt'  style=\"width:395px;\"  ></div></div>");
    },
	load:function(isHasImpNotice){ 
		// alert('3343545');
        var date=new Date();
        jQuery.ajax({
      ...
      var result=mc.analyseM(response,isHasImpNotice) ;
      var msgs=result[7];
     	jQuery("#posNews").html(msgs);
      if(result[6]==false) return;  
      var mstr=result[0];
      var title=result[1];
     ...
     // alert(jQuery("span[class=ui-icon ui-icon-closethick]").html());
      //Query("span[class=ui-icon ui-icon-closethick]").html("X");
      jQuery("#ui-dialog-title-dialog>input[type='button']").css("display","none");
      ...
      if(!mc.modal){
          mc.timeoutId=setTimeout("mc.fadeMessage();",8000);
          jQuery("#dialog").parent().mouseenter(function(){if(mc.timeoutId!=null)clearTimeout(mc.timeoutId);});
         }
        // alert(jQuery("div[tabIndex=-1]").attr("role"));
       
			
		}, 
		error:function(xhr) {}
		});//end of ajax call
		//alert(jQuery("div[class][role=dialog]").attr("role"));
		
		//jQuery(".ui-dialog ui-widget ui-widget-content ui-corner-all")[0].css("display","block");
	 },  //end of load dialog
	refresh:function(){
		var date=new Date();
		jQuery.ajax({
	...
	analyseM:function(response,isHasImpNotice){
			var root = response.documentElement;
			
			// loop through all tree children
			var cs = mc.getChildren(root,"message");
			var noshow=mc.getChildren(root,"noshow");
			var str="<div style='height:130px;overflow-y:scroll;overflow-x:visible;'><table cellspacing='0' cellpadding='0' ><thead><tr><th></th><th>"+"\u4f18\u5148\u7ea7"+"</th><th>"+"\u53d1\u9001\u65f6\u95f4"+"</th><th>"+"\u7f16\u53f7"+"</th><th>"+"\u6807\u9898"+"</th></tr></thead><tbody>";
		...
		//	 msgs+="	&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='#' onclick='javascript:bpos.lineShop("+10084+",11"+")'>"+mc.getChildValue(cs[i],"TITLE")+"</a>&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  ";	      
  		...
  		str+="</tbody></table></div>";
			...
			var shortTitle="";
			if (count>0){
				title+="  &nbsp; &nbsp;<span id='dialog-title' style='color:red;'>"+"\u4f60\u6709$$\u6761\u7d27\u6025\u6d88\u606f\u5fc5\u9700\u9a6c\u4e0a\u5904\u7406".replace('$$',count)+"!!!</span>";
				shortTitle="\u4f60\u6709$$\u6761\u7d27\u6025\u6d88\u606f\u5fc5\u9700\u9a6c\u4e0a\u5904\u7406".replace('$$',count)+"!!!";}
			else{
				 title+="  &nbsp; &nbsp;<span id='dialog-title'>"+"\u5171\u6709$$\u6761\u9700\u8981\u786e\u8ba4".replace('$$',cs.length)+".</span>";			
			   shortTitle="\u5171\u6709$$\u6761\u9700\u8981\u786e\u8ba4".replace('$$',cs.length)+".";
			}
			title+="&nbsp;<input type='button' value='"+"\u5168\u90e8\u901a\u544a"+"' style='vertical-align:middle;padding-top:4px;cusor:pointer;position:relative;left:"+((count>0)?"50":"120")+"px;' onclick='mc.go();'>";
			mc.modal=(count>0);
			mc.mcount=cs.length;
			return [str,title,shortTitle,cs.length,count,noshow.length,isHasImpNotice,msgs];
