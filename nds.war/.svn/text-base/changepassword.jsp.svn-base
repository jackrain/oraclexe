...
int tableId= table.getId();
    ...
    ResultSet result= QueryEngine.getInstance().doQuery("select name, truename, passwordhash from users where id="+userid);
    if(!result.next()) {
  ...
  String oldpassword=result.getString(3);
...
<p>
<form name="password_modify" method="post" action="/changepassword.jsp?objectid=<%=userid%>" >
<br>
