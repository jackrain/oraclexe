<%
/**
 if only one column displayed, will not add div to the element
*/
try{
%>
<%@ include file="/list/data_list.jsp" %>
    ...
    <%
        ...
        for(int i=1;i< meta.getColumnCount();i++){ // first column should always be PK
            ...
            if(url!=null) columnData="<a "+ 
            	...
            	if(columnDataShort.length()!=columnData.length()){
            		columnData="<span title='"+ columnData+"'>"+columnDataShort+"</span>";
            	}
            }
...
%>
		<div class="<%=cls%>"><a class="action" href="<%=moreURL%>"><%=LanguageUtil.get(pageContext, "text-more")%></a></div>
<%				
	}
%>
