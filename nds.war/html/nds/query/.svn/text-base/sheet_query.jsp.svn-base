...
<%
     ...
     *  4. "status " show what type status(int)
     *  5  "action"  string "input" | "view"
     */
%>
<%
	...
	int status= ParamUtils.getIntAttributeOrParameter(request, "status", -1);
	String action=request.getParameter("action");
	boolean showStatusColumn= (status !=-1);
	if ( !showStatusColumn) status=1; // default to 1 that means draft
...
<%@ include file="js/functions.js" %>
 <Script language="javascript">
  ...
  function changeTabCount(cnt){
    window.location="<%=request.getContextPath()%>/query/sheet_query.jsp?table=<%=table.getName()%>&return_type=<%=return_type%>&accepter_id=<%=accepter_id%>&tab_count="+cnt+"&action=<%=action%>" ;
  }
</script>
<br>
  ...
  <table border="1" cellspacing="0" cellpadding="0" align="center" bordercolordark="#FFFFFF" bordercolorlight="#FFCC00" width="100%">
    ...
    <td >
      <table width="98%" border="0" cellspacing="0" cellpadding="0" align="center">
        ...
        <%
                               ...
                               for(int iTab_count=1;iTab_count< 10;iTab_count++){
                                   otab_count.put(iTab_count+"页", ""+iTab_count);
                               }
        ...
        %>
            ...
            &nbsp;&nbsp;&nbsp;<a href="#" class="DropDown" onclick="Outline2()"><img border=0 src="../images/blueup.gif" >提示</a>
            <DIV id="ExpCol"  class="collapsed" border="0" ><font color=red>*</font>
            条件页之间是平级的，可以设置“并且、或者、并且不、或者不”等关系;条件页内的字段之间是“并且”关系。<p>例如，要找到产品单价小于100或者进货年季为200103的产品，就需要建立两个分页，第一分页设置单价小于100, 第二分页设置进货年季为200103，第二分页上的条件为“或者包含以下条件”。<p>有时为了在一个字段上设置多个条件，也需要采用分页，如查找单价在100和200之间的产品。
          </DIV>
          </td></tr></table>
      <%
      ...
      <form name="<%=form_name %>" method="post" action="<%= request.getContextPath()+"/servlets/QueryInputHandler"%>" onSubmit="return doSubmit(document.<%=form_name %>);" >
        ...
        <input type='hidden' name='resulthandler' value='/objext/sheet_list.jsp<%=(showStatusColumn?"?status="+status:"")%>'>
                    ...
                    <td colspan=2 height="20" background="../images/Nds_table_bg_2.gif">
						...
						<b>分页<%=tabIdx+1%></b>&nbsp;&nbsp;&nbsp;
						   	<% if( tabIdx !=0){
						   		...
						   		osql_comb.put( "并且满足以下条件",""+SQLCombination.SQL_AND);
						   		osql_comb.put( "或者满足以下条件",""+SQLCombination.SQL_OR);
						   		osql_comb.put( "并且不包含以下条件",""+SQLCombination.SQL_AND_NOT);
						   		osql_comb.put( "或者不包含以下条件",""+SQLCombination.SQL_OR_NOT);
