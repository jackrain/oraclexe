...
"SELECT getakno(item.productid,'product') productno, '滞后盘点' dtype, qty, no, '滞后盘点' sheettype, 'N/A' peer, to_char(checkdate,'YYYY-MM-DD HH24:MI:SS') theDay, 'N/A' ap"+
"	from  monthcustcheckshtitem item, monthcustchecksht sht where "+
"	item.monthcustcheckshtid= sht.id and item.productid in (select id from product where no like '&2%') "+
"	and item.customerid=(select id from customer where no='&1') "+
"union all	"+
"select productno, '客户收货' dtype,RECQTY qty, sheetno,"+
...
"  from outletrecftp where RECCUSTNO ='&1' and productno like '&2%'"+
"  and recdate >= 20031201"+
" union all  "+
"  select productno, '销售', -qty, OUTLETDAYSALESHTNO, '销售', null, "+
"  to_char(DAYSALEDATE,'YYYY-MM-DD HH24:MI:SS'), null from outletdaysaleftp"+
"  where CUSTOMERNO='&1' and productno like '&2%'"+
" UNION ALL"+
"  select productno,'盘点误差', ADJQTY, CHEQTYERRADJSHTNO, '盘点误差', null, "+
"  to_char(SHEETSUBMITDATE,'YYYY-MM-DD HH24:MI:SS'), to_char(ap) from CheQtyErrAdjFtp"+
"  where OUTLETWHNO='&1' "+
"  and productno like '&2%'"+
" union all"+
"  select getakno(item.productid,'product'),'当前库存',   OUTLETSTORAGE, 'N/A', '当前库存', null, null, null from"+
"  outletstorage where customerid=(select id from customer where no='&1') "+
"  and productid in (select id from product where no like '&2%') "+
" union all"+
"  select getakno(item.productid,'product'),'有POS盘点库存', checkqty, no,    '盘点库存', null,"+
"  to_char(modifieddate,'YYYY-MM-DD HH24:MI:SS'), 'N/A' from checksht sht, checkshtitem item"+
"  where shopid=(select id from customer where no='&1') and productid in (select id from product where no='&2%')"+
"  and item.checkshtid=sht.id "+
" union all"+
"  select getakno(item.productid,'product'),'盘点库存', CHESTORAGE, no,    '盘点库存', null,"+
"  to_char(modifieddate,'YYYY-MM-DD HH24:MI:SS'), 'N/A' from outletcheqtyerrsht sht, outletcheqtyerrshtitem item"+
"  where customerid=(select id from customer where no='&1') and productid in (select id from product where no='&2%')"+
