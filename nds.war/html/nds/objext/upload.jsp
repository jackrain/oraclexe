...
<%
	...
	if (holder != null) {
		savedURL = (String)holder.get("url");
...
<div id="tabs">
	...
	<div id="tab1">
		...
		<table style="word-break:break-all;" border="0" cellspacing="0" cellpadding="0" align="center" bordercolordark="#FFFFFF" bordercolorlight="#999999" width="100%">
			<tr>
				<td> 
					<table style="white-space:nowrap;" border="0" cellspacing="5" cellpadding="0" align="center" width="90%">
						<tr>
							<br>
							<td><b><%=PortletUtils.getMessage(pageContext, "referred-by",null)%>:</b></td>
							<td>
								<a href="<%=NDS_PATH+"/object/object.jsp?table="+tableId+"&id="+objectId%>"><%=NDS_PATH+"/object/object.jsp?table="+tableId+"&id="+objectId%></a>
								[<%=table.getDescription(locale)+"-"+ col.getDescription(locale)%>]
							</td>
						</tr>
						<tr>
							<td><b><%=PortletUtils.getMessage(pageContext, "file-history",null)%>:</b></td>
							<td>
						...
						<% if(atts.size()>0){%>
						<tr>
							<td><b><%=PortletUtils.getMessage(pageContext, "file-ext",null)%>:</b></td>
							<td align='left'><%=( (Attachment)atts.get(0)).getExtension()%></td>
						</tr>
						<%}%>
						<tr>
							<td></td>
							...
							<%	if ((objPermission & 0x3) == 3) { %>
								<form name="sheet_upload_form2" method="post" action="<%=request.getContextPath() %>/control/command">
						...
						<tr>
							<td colspan=2>
								<br>
								<%=PortletUtils.getMessage(pageContext, "upload-guide",null)%>
								...
								<form id="sheet_upload_form" name="sheet_upload_form" method="post" enctype="multipart/form-data" action="<%=request.getContextPath() %>/control/upload">
									<input type='hidden' name="directory" value='<%=table.getSecurityDirectory()%>'>
									<input type="hidden" name="table" value="<%=tableId %>">
									<input type="hidden" name="column" value="<%=columnId %>">
									<input type="hidden" name="objectid" value="<%=objectpath==null?objectId:objectpath%>">
									...
									<input type="text" name="fileurl" id="fileurl" maxlength="300" size="50" >
									--><p>
									<!--%}%-->
									<div class="buttons">  
		...
		<script>checkUpload(sheet_upload_form);</script>
	</div>
</div>
