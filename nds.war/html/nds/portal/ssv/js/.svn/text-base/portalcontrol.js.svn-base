...
PortalControl.prototype = {
	initialize: function() {
		...
		this._cxtabUserDims=0; //user dim limitation
...
/**
	...
	*/
    ...
    limitUserDims:function(cxtabId, userDims){
    	this._cxtabId=cxtabId;
    	this._cxtabUserDims=userDims;
    },
    ...
    doReportNow:function(){
    	return this._doReportNow;
    },
    setDoReportNow:function(b){
    	this._doReportNow=b;
    },
    /**
     * @param filetype html (default) or xls
	...
	*/
    ...
    doCancatPDF:function(){
		var evt={};
		evt.tag="Print";
		evt.command="PrintJasper";
		evt.callbackEvent="PrintJasper";
		evt.params={"table":this._tableObj.id,"id":this._getSelectedItemIds()};	
		if(evt.params.id.length==0){
 			alert(gMessageHolder.PLEASE_CHECK_SELECTED_LINES);
            return;				
		}
		this.executeCommandEvent(evt);
...
IMClient.prototype = {
	...
	show:function(idx){
		...
		if(obj.noteid!=null && !isNaN(obj.noteid)){
			msg+="<div class='imf'><a href='javascript:dlgo(10083,"+obj.noteid+")'>"+gMessageHolder.VIEW_NOTE+"</a></div>";
		}
		msg+="</div>";
    	var html=msg +"<br><div id='cmdbtns'><input type='button' value='&nbsp;&nbsp;"+gMessageHolder.CONFIRM_READ+"&nbsp;&nbsp;' onclick='im.dlgok("+idx+")'></div>";
		pc.cmdmsgbox(html);
	},
	/**
	...
	check:function(){
		new Ajax.Request("/servlets/binserv/Messages?lastid="+ im._lastId, {
		  ...
		  onSuccess: function(transport) {
		  	...
		  	if(newmsgs==null || newmsgs.length==0) return;
		  	var oldIdx=im._msgs.length;
		  	var oldLastId=im._lastId;
		  	im._msgs=im._msgs.concat(newmsgs);
		    ...
		    for(idx=0;idx< newmsgs.length;idx++){
		    	obj= newmsgs[idx];
		    	new Insertion.Top('imul', "<li><a id='mi"+(oldIdx+idx)+"' href='javascript:im.show("+(oldIdx+idx)+")' class='unread "+(obj.css==null?"":obj.css)+"'>"+obj.msg+"</a></li>");
		    	if(obj.block) blockerIdx=oldIdx+idx;
		    }
