...
%> <tree text="<%=desc%>" src="<%=src%>" action="javascript:cxtabDefControl.setColumn('<%=clink%>','<%=akDesc%>')"/>
<%	}else{
		if(col.getSubTotalMethod()!=null){
%> <tree text="<%=desc%>" icon="<%=NDS_PATH%>/images/sum-column.gif" action="javascript:cxtabDefControl.setColumn('<%=clink%>','<%=parentDesc+desc%>')" />
<%	
		}else{	
%> <tree text="<%=desc%>" action="javascript:cxtabDefControl.setColumn('<%=clink%>','<%=parentDesc+desc%>')"/>
<%	
		}
	}
}%></tree>
