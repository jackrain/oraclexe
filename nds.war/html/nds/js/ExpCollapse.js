...
function ExpandAll()
{
	...
	if (sCaption==L_EXPAND_TEXT)
		{
		sAction="expanded";
		sImage="/html/nds/images/bluedrop.gif"
		document.all("picHeader").src="/html/nds/images/collapsetri.gif";
		document.all("ExpandAll").innerHTML=L_COLLAPSE_TEXT;
		}
	else
		{
		sAction="collapsed";
		sImage="/html/nds/images/blueup.gif"
		document.all("picHeader").src="/html/nds/images/expandtri.gif";
...
function Outline2()
{
	...
	if(document.all(elemPos+incr).className=="collapsed")
	{
		...
		document.all(elemPos+incr).className="expanded"
		document.all(elemPos+1).src="/html/nds/images/bluedrop.gif";
		if(open.tagName=="IMG"){open.src="/html/nds/images/bluedrop.gif";}
		if(open.tagName=="B")
			{
			if(open.parentElement.all.tags("IMG").length != 0)
				{open.parentElement.all.tags("IMG").item(0).src="/html/nds/images/bluedrop.gif";}
			}
	}
	else if(document.all(elemPos+incr).className=="expanded")
	{
		document.all(elemPos+incr).className="collapsed"
		document.all(elemPos+1).src="/html/nds/images/blueup.gif";
		if(open.tagName=="IMG"){open.src="/html/nds/images/blueup.gif";}
		if(open.tagName=="B")
			{
			if(open.parentElement.all.tags("IMG").length != 0)
				{open.parentElement.all.tags("IMG").item(0).src="/html/nds/images/blueup.gif";}
			}
	}
	else
	{
...
//elemPos = window.event.srcElement.sourceIndex;
	...
	if (defSpan && document.readyState == "complete" && theIFRAME.document.readyState == "complete"){
			...
			document.all(elemPos+1).src="/html/nds/images/bluedrop.gif";
			document.all(elemPos).src="/html/nds/images/bluedrop.gif";
		}
    		else{
			defSpan.style.display = "none";
			defSpan.parentElement.style.display = "none";
			document.all(elemPos+1).src="/html/nds/images/blueup.gif";
			//if(oSource.tagName=="IMG"){open.src="/html/nds/images/blueup.gif";}
    		}
	}
}
