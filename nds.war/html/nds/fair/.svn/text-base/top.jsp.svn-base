...
<title>Portal</title>
<link href="/html/nds/fair/base.css" rel="stylesheet" type="text/css" />
<script language="JavaScript" src="/html/nds/js/prototype.js"></script>
<script type="text/javascript" src="/html/nds/fair/jquery.js"></script>
<script type="text/javascript" src="/html/nds/fair/jquery.jcarousel.pack.js"></script>
<script>
	jQuery.noConflict();
</script>	
...
<script language="javascript" src="/html/nds/js/fair.js"></script>
<script src="/html/nds/fair/outline.js"></script>
</head>
<% 
...
if(fairlist.size()==0){
    	response.sendRedirect("/html/nds/fair/nofairinfo.jsp");
}
String currfairname ="";
  if(fairid==-1){
...
<body><div id="hdr_topsec"><div id="hdr_mainmsg">
	<a href="/html/nds/fair/index.jsp" class="hdr_logo" title="logo">logo</a>
		<div id="channel">
			<div class="wel_msg"><%=userWeb.getUserDescription()%>，您好，您当前的主题是：<%=currfairname%></div>
			<ul>
				<li><a href="/html/nds/fair/index.jsp" title="订货会首页">订货会首页</li>
				<li><a href="/html/nds/portal/portal.jsp" title="业务系统">业务系统</a></li>
				<li><a href="/html/nds/fair/my_order.jsp?fairid=<%=fairid%>" title="我的订单">我的订单</a></li>
				<li><a href="<%= themeDisplay.getURLSignOut() %>" title="退出"><bean:message key="sign-out" /></a></li>
		  </ul>
		</div>
		<div id="buycart" class="buycart">
			<span id="itemnum">您的<a href="/html/nds/fair/my_order.jsp?fairid=<%=fairid%>" title="订单袋">订单袋</a>有<span class="txt_highlight bold" id="totqty">0</span>件物品</span>
	  </div>
  </div>
</div>
<div id="hdr_tabbar">
	<div id="hdr_maintab">
<span><center><a href="/html/nds/fair/index.jsp?fairid=<%=fairid%>" target="_self"><%=currfairname%></a></center></span>
<ul>
	<%
	for(int i=0;i<fairlist.size();i++){
	if(Tools.getInt(((List)fairlist.get(i)).get(0),-1)!=fairid){
	%>
	<li><a href="/html/nds/fair/index.jsp?fairid=<%=Tools.getInt(((List)fairlist.get(i)).get(0),-1)%>" target="_self" ><%=((List)fairlist.get(i)).get(1)%></a></li>
	<%}}%>
</ul>
</div>
</div>